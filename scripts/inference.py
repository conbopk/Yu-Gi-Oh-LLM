...
from llm_from_scratch.tokenizer import TokenizerConfig, TokenizerType, CharTokenizer, BPETokenizer, CARD_START, CARD_END
...
def sample_next_token(model: Transformer, input_tokens: Tensor, temperature: float) -> Tensor:
    ...
    with torch.no_grad():
        ...
        # 1 x V
        logits = logits[:, -1, :]   # last generated token
...
if len(input_tokens) > model_config.context_length:
    raise ValueError(f"{len(input_tokens)} input tokens > {model_config.context_length} model context length")
