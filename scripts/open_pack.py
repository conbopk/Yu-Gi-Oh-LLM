...
from llm_from_scratch.tokenizer import CARD_START, CARD_END, TokenizerConfig, TokenizerType, CharTokenizer, BPETokenizer
from llm_from_scratch.transformer import Transformer, TransformerConfig
from llm_from_scratch.utils import fetch_device

# --- Params ---

MODEL_PATH = "model/2026-07-03_19-56-20/best_model.pt"
PACK_SIZE = 5
...
def sample_next_token(model: Transformer, input_tokens: Tensor, temperature: float) -> Tensor:
    if temperature <= 0:
...
def generate_card(model: Transformer, tokenizer, device: str, context_length: int) -> str:
    ...
    for _ in range(MAX_TOKENS_TO_GENERATE):
        ...
        next_token = sample_next_token(model=model, input_tokens=cur_context, temperature=TEMPERATURE)
...
def print_card(card_text: str, index: int, total: int):
    ...
    body_lines = [
        line
        for line in card_text.splitlines()
        if line not in (CARD_START, CARD_END) and not line.startswith("name:") and not line.startswith("rarity:")
    ]
    ...
    for line in body_lines:
        for wrapped_line in textwrap.wrap(line, width=width) or [""]:
            field = next(
                (
                    field_name
                    for field_name in FIELD_NAMES if wrapped_line.startswith(f"{field_name}:")
                ),
                None,
            )
            if field:
