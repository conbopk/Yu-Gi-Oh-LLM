...
from llm_from_scratch.checkpoint import save_checkpoint, load_checkpoint
from llm_from_scratch.dataset import YuGiOhCardsDataset
from llm_from_scratch.tokenizer import TokenizerConfig, TokenizerType, Tokenizer, CharTokenizer, BPETokenizer
from llm_from_scratch.transformer import TransformerConfig, Transformer
...
TRAIN_CONFIG = TrainConfig(
    ...
    wandb_project="yu-gi-oh-llm",
)

MODEL_CONFIG = TransformerConfig(
    embedding_dim=512 // 4,      # Attention is all you need paper
    context_length=128,     # Attention is all you need paper
    attention_heads=8 // 2,      # Attention is all you need paper
    ff_hidden_dim=2048 // 2,     # Attention is all you need paper
    n_decoders=6 // 2,           # Attention is all you need paper
...
def train(
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        for batch, (X, y) in enumerate(train_dataloader):
            ...
            if global_step % log_every_steps == 0:
                ...
                tokens_per_second_str = (f"{tokens_per_second / 1000:.1f}k" if tokens_per_second >= 1000 else f"{tokens_per_second:.0f}")
    ...
    print(f"[final] step={global_step} val_loss={last_val_loss:.4f} ppl={perplexity:.2f}")
    if wandb_run is not None:
        wandb_run.log(
            {
...
def lr_schedule(step: int) -> float:
    ...
    return (MODEL_CONFIG.embedding_dim**-0.5) * min(step**-0.5, step * TRAIN_CONFIG.warmup_steps**-1.5)
