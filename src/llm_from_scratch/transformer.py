...
from torch.nn import Module, Embedding, Dropout, ModuleList, Linear, LayerNorm, ReLU
...
class Attention(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.masked:     # can't attend future tokens
            T = x.shape[1]
            q_kt = q_kt.masked_fill(self.mask[:T, :T], float("-inf")) # -inf instead of 0 as we can have negatives
...
class MultiHeadAttentionSeq(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # B x T x C
        x = torch.concat(tensors=x_heads, dim=-1)
        # B x T x C
        return self.wo(x)
